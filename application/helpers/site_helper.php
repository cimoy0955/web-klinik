	...
	function get_site_menus($parent = 0)
	{
		...
		foreach ($menus as $menu) {
			...
			if (count($cek_parents) > 0) {
				...
				$html .= '
				<li class="nav__item has-dropdown"><a class="dropdown-toggle nav__item-link" href="' . $url . '">' . ucwords($menu->title) . ' 
				...
				$html .= '<li class="nav__item"><a class="nav__item-link" href="' . $url . '">' . ucwords($menu->title) . '</a>' . get_site_menus($menu->post_id) . '</li>';
			}
		}
